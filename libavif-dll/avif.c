...
DllExport const char * avifVersion(void)
{
    return AVIF_VERSION_STRING;
}

DllExport const char * avifPixelFormatToString(avifPixelFormat format)
{
    switch (format) {
...
DllExport void avifGetPixelFormatInfo(avifPixelFormat format, avifPixelFormatInfo * info)
{
...
DllExport const char * avifResultToString(avifResult result)
{
    // clang-format off
    switch (result) {
...
DllExport avifImage * avifImageCreate(int width, int height, int depth, avifPixelFormat yuvFormat)
{
...
DllExport avifImage * avifImageCreateEmpty(void)
{
    return avifImageCreate(0, 0, 0, AVIF_PIXEL_FORMAT_NONE);
}

DllExport void avifImageCopy(avifImage * dstImage, const avifImage * srcImage, uint32_t planes)
{
...
DllExport void avifImageDestroy(avifImage * image)
{
...
DllExport void avifImageSetProfileICC(avifImage * image, const uint8_t * icc, size_t iccSize)
{
    avifRWDataSet(&image->icc, icc, iccSize);
}

DllExport void avifImageSetMetadataExif(avifImage * image, const uint8_t * exif, size_t exifSize)
{
    avifRWDataSet(&image->exif, exif, exifSize);
}

DllExport void avifImageSetMetadataXMP(avifImage * image, const uint8_t * xmp, size_t xmpSize)
{
    avifRWDataSet(&image->xmp, xmp, xmpSize);
}

DllExport void avifImageAllocatePlanes(avifImage * image, uint32_t planes)
{
...
DllExport void avifImageFreePlanes(avifImage * image, uint32_t planes)
{
    if ((planes & AVIF_PLANES_YUV) && (image->yuvFormat != AVIF_PIXEL_FORMAT_NONE)) {
...
DllExport void avifImageStealPlanes(avifImage * dstImage, avifImage * srcImage, uint32_t planes)
{
...
DllExport avifBool avifImageUsesU16(const avifImage * image)
{
...
DllExport void avifCodecDestroy(avifCodec * codec)
{
...
DllExport avifBool avifRGBFormatHasAlpha(avifRGBFormat format)
{
    return (format != AVIF_RGB_FORMAT_RGB) && (format != AVIF_RGB_FORMAT_BGR);
}

DllExport uint32_t avifRGBFormatChannelCount(avifRGBFormat format)
{
    return avifRGBFormatHasAlpha(format) ? 4 : 3;
}

DllExport uint32_t avifRGBImagePixelSize(const avifRGBImage * rgb)
{
    return avifRGBFormatChannelCount(rgb->format) * ((rgb->depth > 8) ? 2 : 1);
}

DllExport void avifRGBImageSetDefaults(avifRGBImage * rgb, const avifImage * image)
{
...
DllExport void avifRGBImageAllocatePixels(avifRGBImage * rgb)
{
    if (rgb->pixels) {
        avifFree(rgb->pixels);
    }
...
DllExport void avifRGBImageFreePixels(avifRGBImage * rgb)
{
    if (rgb->pixels) {
        avifFree(rgb->pixels);
    }
...
DllExport avifCodecSpecificOptions * avifCodecSpecificOptionsCreate(void)
{
...
DllExport void avifCodecSpecificOptionsDestroy(avifCodecSpecificOptions * csOptions)
{
    if (!csOptions) {
        return;
    }
...
DllExport void avifCodecSpecificOptionsSet(avifCodecSpecificOptions * csOptions, const char * key, const char * value)
{
...
DllExport typedef const char * (*versionFunc)(void);
DllExport typedef avifCodec * (*avifCodecCreateFunc)(void);

DllExport struct AvailableCodec
{
...
DllExport const char * avifCodecName(avifCodecChoice choice, uint32_t requiredFlags)
{
...
DllExport avifCodecChoice avifCodecChoiceFromName(const char * name)
{
    for (int i = 0; i < availableCodecsCount; ++i) {
...
DllExport avifCodec * avifCodecCreate(avifCodecChoice choice, uint32_t requiredFlags)
{
    struct AvailableCodec * availableCodec = findAvailableCodec(choice, requiredFlags);
    if (availableCodec) {
...
DllExport void avifCodecVersions(char outBuffer[256])
{
